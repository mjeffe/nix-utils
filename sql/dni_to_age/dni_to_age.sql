...
--
-- Parameters:
--  doc_type   the document type.  There are several such as passport, DL, etc.
...
--  20-JUN-2000  mjeffe   added a check to convert to number, before
--                        going into the age calc section.  This should
...
begin
   -- if doc_type is not DNI then it won't work
   if doc_type != 'DNI' then return null;
   end if;

   -- try to convert the doc_num to a number
   declare
      tmp_num number;
   begin
      tmp_num := to_number(doc_num);
   exception
      when VALUE_ERROR then
      begin
         -- a vlaue error is raised when the to_number fails to convert the varchar
         -- to a number.  This will be due to garbage in the doc_num field.
         return null;
      end;
   end;

   -- we have a valid DNI number, so calculate the age
   if    ( doc_num >= 41541000 ) then return                         '0-1  ';
   ...
   -- there are gaps in some of these ranges, so if it can't calculate
   -- an age then return null
   else return null;
   end if;

exception
   when OTHERS then
      DBMS_OUTPUT.PUT_LINE('dni_to_age: unknown error occured');
